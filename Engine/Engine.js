		...
		if(current_player.currentRoom.doors[i] == xroom){
			...
			if(current_player.currentRoom.events != false){
				if(current_player.currentRoom.events == "sticks"){
					document.getElementById("myDiv").innerHTML = document.getElementById("myDiv").innerHTML + ". You step on some sticks as you walk in."
					play_sound(0);
				}
				else if(current_player.currentRoom.events == "skeletonScream"){
					current_player.currentRoom.events = "skeletonNoScream"
					document.getElementById("myDiv").innerHTML = document.getElementById("myDiv").innerHTML + ". You walk into the room and see the skeleton. You scream in fright."
					play_sound(1);
				}
				else if(current_player.currentRoom.events == "skeletonNoScream"){
					document.getElementById("myDiv").innerHTML = document.getElementById("myDiv").innerHTML + ". You walk into the room and see the skeleton. You calm yourself, as you saw it before."
...
var search_skelly = function(){
	if (current_player.currentRoom.name == bedroom) {
		if(skelly){
			document.getElementById("myDiv").innerHTML = "You search the skeleton. You find a Death Claw."
			search_skelly = false;
			add_inventory(current_player.name, "Death Claw");
		}
		else{
			document.getElementById("myDiv").innerHTML = "You already searched the skeleton. You find nothing new."
		}
	}
}
...
var show_inventory = function(){
	if (current_player.inventory == 0) {
		document.getElementById("myDiv").innerHTML = "You have nothing in your inventory."
	}
	else{
